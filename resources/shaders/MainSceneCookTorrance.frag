...
void main()
{
    ...
    /// Forward+ branch
    uvec2 tileIndexD2 = uvec2(uint(gl_FragCoord.x) / m_tileSize.width,
                              (m_resolution.height - uint(gl_FragCoord.y)) / m_tileSize.height); // Flip the y, for the default FBO uses bottom left as origin
    ...
    //if (tileIndexD2.x > 80 || tileIndexD2.y > 45) { outColor = vec4(1.0f, 0.0f, 0.0f, 1.0f); }
    //else { outColor = vec4(0.0f, 1.0f, 0.0f, 1.0f); }
}
